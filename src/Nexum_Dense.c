 ...
 * @return void
 *
 * @see Nexum_Activation, Nexum_Tensor_init_randn(), Nexum_Tensor_init_zeros().
 */
void Nexum_Dense_init(Nexum_Dense* L, u64 in_features, u64 out_features, Nexum_Activation act) {
	L->in_features = in_features;
	L->out_features = out_features;
	L->act = act;
	L->initialized = true;

	Nexum_Tensor_init_randn(&(L->weights), out_features, in_features);
	Nexum_Tensor_init_zeros(&(L->bias), out_features, 1);
}
...
void Nexum_Dense_read(Nexum_Dense* L, str fname) {
	...
	Nexum_Dense_init(L, in_features, out_features, act);
